 ...
 * @since      1.0.0
 */
class videolife_Random_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-videolife-random widget-posts-thumbnail',
			'description' => __( 'Display random posts with thumbnails.', 'videolife' )
		);

		// Create the widget.
		parent::__construct(
			'videolife-random',                                   // $this->id_base
			__( '&raquo; Random Posts', 'videolife' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		if ( false === ( $random = get_transient( 'videolife_random_widget_' . $this->id ) ) ) {
			...
			// Store the transient.
			set_transient( 'videolife_random_widget_' . $this->id, $random );
	...
	function flush_widget_transient() {
		delete_transient( 'videolife_random_widget_' . $this->id );
	}
	...
	function form( $instance ) {
		...
		// Default value.
		$defaults = array(
			'title'     => esc_html__( 'Random Posts', 'videolife' ),
			'limit'     => 5,
			'show_date' => true
		);
